...
std::string EBook::GetLink() const { return download_link; }

std::string EBook::GetFormat() const { return format; }
...
std::string EBook::ToCsv() const {
  // format the float number to avoid loss of precision
  char buffer[15];
  sprintf(buffer, "%.1f", file_size);
  std::string str_file_size = buffer;
